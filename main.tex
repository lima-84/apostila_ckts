...
\begin{itemize}
    ...
    \item \hspace{0.2cm} $P_k < 0$: A potência é \textbf{fornecida} quando as cargas elétricas passam de um ponto com \textbf{menor} potencial elétrico $(-)$ para um de \textbf{maior} potencial elétrico $(+)$. Neste caso, pode-se dizer que o elemento \textbf{orneceu} potência elétrica para o sistema.
\end{itemize}
...
A expressão \eqref{eq:edol_exemplo} é uma equação diferencial ordinária linear com coeficientes constantes de ordem 2 cuja incógnita é a função $e_{o}(t)$. Para o circuito em questão, substituindo os valores, tem-se
...
\noindent Tais condições iniciais, juntamente com a equação diferencial, formam um \textbf{problema de valor inicial (PVI)}. Em sumo, para encontrar a solução única $e_o(t)$, basta resolver
...
\noindent Como já mencionado, a dependência do tempo será omitida nos exemplos subsequentes, bem como a condição $t \geqslant 0$, pois, a menos que explicitado, os circuitos serão a partir do instante em que a fonte de alimentação é ligada.
...
\begin{gather*}
    ...
    \\
    y_2(t) = \dfr{z_1(t)-z_2(t)}{2j} = e^{\sigma t}\sin(\omega t).
\end{gather*}
...
Retornando ao PVI \eqref{eq:pvi}, separa-se a resposta natural da forçada:
\vspace{2mm}
