...
\subsection{Comentários dos Autores}
\label{subsec:coment}
...
Quaisquer erros encontrados ou sugestões sobre esse material, favor contatar pelo seguinte email: \textbf{ceeca.box@gmail.com}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \begin{axis}
                ...
                \addplot [domain=-pi:0,red,thick] {0};
                \addplot[domain=-pi:4*pi,red,thick] coordinates {(0,0)(0,1)};
                \addplot [domain=0:4*pi,red,thick] {1};
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \begin{axis}
                ...
                \addplot [domain=-pi:0,red,thick] {0};
                \addplot [domain=-pi:0,blue,thick] {0};
                \addplot[domain=-pi:4*pi,red,thick] coordinates {(0,0)(0,1)};
                \addplot[domain=-pi:4*pi,blue,thick] coordinates {(0,0)(0,1)};
                \addplot [domain=0:4*pi,red,thick] {1};
                \addplot [domain=0:4*pi,blue,thick] {cos(x*360/(2*pi))};
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \begin{axis}
                ...
                \addplot[domain=-1:1,red,very thick] coordinates {(0,0)(0,1)};
                \addplot[domain=-1:1,red,very thick,->] coordinates {(0,0)(0,1)};
...
\noindent Portanto, as Equações \eqref{eq:e1g7.5} e \eqref{eq:e1l30} explicitam o intervalo de $e_1$ em que cada um dos diodos está no estado da hipótese, supondo que o outro também esteja. Então, o intervalo em que de fato a hipótese é válida consiste na intersecção desses intervalos.
...
\end{minipage} \hfill
\begin{minipage}[c][3cm][c]{\dimexpr0.4\textwidth-0.5\Colsep\relax}
    Assim, é possível descobrir a corrente que entra na fonte de $-15V$, à qual será denominada como $i_{R_3}$.
\end{minipage}
\end{minipage}
...
\begin{minipage}[c][7cm][c]{\dimexpr0.6\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}\draw
            ...
            (2,2) to[short,-*] (2,2.5)
            (4,2) to[short,-*] (4,2.5)
            (3,0) node[tlground]{} to[R,l_=$R_0$,v_=$-e_0$,-*] (3,2) node[label={above:$e_0$}]{}
            ;
        \end{tikzpicture}
    \end{center}
\end{minipage} \hfill
\begin{minipage}[c][7cm][c]{\dimexpr0.4\textwidth-0.5\Colsep\relax}
    Voltando ao circuito anterior, é possível descobrir a expressão de $i_{D_1}$ em função de $e_1$ descobrindo a tensão $-e_1$, ao fazer a malha que percorre $R_0$,$R_1$ e $e_1$, e dividir pela resistência $R_0$. Para a tensão $V_{D_2}$ se percorre a malha contendo $R_0$,$V_{D_2}$, $R_3$ e fonte de $-15V$. Em ambos os casos, aplica-se as condições verificadoras.
\end{minipage}
\end{minipage}
...
\end{minipage} \hfill
\begin{minipage}[c][3cm][c]{\dimexpr0.4\textwidth-0.5\Colsep\relax}
    Assim, é possível descobrir a corrente que sai da fonte $e_1$, à qual será denominada como $i_{R_1}$.
\end{minipage}
\end{minipage}
...
\end{minipage} \hfill
\begin{minipage}[c][7cm][c]{\dimexpr0.4\textwidth-0.5\Colsep\relax}
    Retornando ao circuito original dentro dessa hipótese, é possível descobrir a expressão de $i_{D_2}$ em função de $e_1$ descobrindo a tensão $e_1$, ao fazer a malha que percorre $R_0$,$R_3$ e a fonte de $-15V$, e dividir pela resistência $R_0$. Para a tensão $V_{D_1}$ se percorre a malha contendo $R_0$,$V_{D_1}$, $R_1$ e fonte $e_1$. Em ambos os casos, aplica-se as condições verificadoras.
\end{minipage}
\end{minipage}
