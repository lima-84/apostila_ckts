...
Fosse relevante, seria possível transformar a fonte de tensão, agora em série com o resistor $R$, em uma fonte de corrente em paralelo sua respectiva carga.
...
Neste caso, a solução geral é dada pela combinação linear não trivial de duas soluções exponenciais linearmente independentes $y_{1,2} = C_{1,2}e^{\lambda_{1,2}t}$, ou seja,
\begin{equation*}
    \displaystyle y_{n}(t)=C_1y_1(t)+C_2y_2(t) = C_1e^{\lambda_1 t}+C_2e^{\lambda_2 t}
\end{equation*}

\itembull{Caso 2:} $\lambda_{1} = \lambda_{2} = \lambda, \quad \lambda_{1},\lambda_{2} \in \mathbb{R}$

Nota-se que as exponenciais escolhidas no caso anterior se tornam linearmente dependentes. Deve-se, portanto, procurar uma outra solução linearmente independente a $y_{1}(t)=e^{\lambda t}$. Através do método de D'Alembert, assume-se que uma solução LI é
...
\begin{equation*}
    \dfr{d^2}{dt^2}y(t)+p\frac{d}{dt}y(t)+qy(t)=x(t)\
\end{equation*}
