...
\begin{itemize}
    ...
    Também é possível usar a função $u(t)$ com objetivo de modular algum outra função, multiplicando-as. O gráfico a seguir mostra um exemplo disso usando uma função trigonométrica:
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \begin{axis}
                ...
                width=0.75\textwidth,
                axis lines = middle,
                x label style={at={(axis description cs:1.05,0.525)},anchor=north},
                y label style={at={(axis description cs:0.2,1.03)},anchor=south},
                minor tick num=1,
                xlabel=$t$,
                ylabel=$\cos(t)u(t)$,
    ...
    A figura a seguir ilustra a representação gráfica de um Delta de Dirac:
...
Uma alternativa de abordagem para resolução desse problema é a de considerar todas combinações possíveis entre os estados dos diodos, dado que cada diodo possui dois estados e existem $N=2$ diodos nesse circuito, o total de combinações a serem analisadas são $2^N=2^2=4$, e, a partir disso, determinar quais delas podem acontecer de fato nesse circuito, para quais valores de $e_1$ acontecem e quais os valores da saída $e_0$ para cada uma delas. Outra abordagem seria a de variar o valor de $e_1$ de $-\infty$ até $\infty$ e, no decorrer desse processo, determinar quais as combinações de estados entre os diodos que ocorrem, bem como a saída $e_0$. Nesse exemplo, será apresentada a primeira abordagem para solucionar o problema.

\vspace{5mm}

\itembull{Hipótese 1:} $\mathbf{D_1}$ e $\mathbf{D_2}$ \textbf{OFF}. Para essa hipótese, as condições verificadoras que a tornam válida são $V_{D_1}<0$ e $V_{D_2}<0$, uma vez que só assim os diodos podem estar no estado de não condução.
...
\begin{minipage}[c][7cm][c]{\dimexpr0.6\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}\draw
            ...
            (0,4) node[label={left:$e_1$}]{} to[R,l=$R_1$,v=$\dfrac{e_1+15}{3}$,*-] (2,4)
            (2,4) to[short,-*] (2,3.5)
            (2,4) to[R,l=$R_2$] (4,4)
            (4,4) to[short,-*] (4,3.5)
...
\begin{minipage}[c][3cm][c]{\dimexpr0.6\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}\draw
            ...
            (2,4) to[R,l=$R_2$,v=$0V$,f<=$0A$,-*] (4,4) node[label={right:$E_{th}$}]{}
...
\end{minipage} \hfill
\begin{minipage}[c][3cm][c]{\dimexpr0.4\textwidth-0.5\Colsep\relax}
    Para encontrar a tensão de Thévenin em um circuito em notação potencial, aplica-se uma tensão de valor $E_{th}$, ainda desconhecida, no ponto de interesse, em que a corrente fornecida é é $0A$, evidenciando que, pela definição, $E_{th}$ é a tensão a circuito aberto. Dessa forma, nesse caso, como a corrente que passa por $R_2$ é nula, os resistores $R_0$ e $R_1$ ficam em série, por conta da mesma corrente percorrer ambos.
\end{minipage}
\end{minipage}

\vspace{2cm}

\noindent Então a tensão $E_{th}$ é justamente, pela malha que percorre $E_{th}$, $R_2$ e $R_0$, a tensão $V_{R_0}$, com o potencial positivo para cima. Uma vez que $R_0$ e $R_1$ estão em série, $V_{R_0}$ é o divisor de tensão da fonte $e_1$ entre os resistores $R_0$ e $R_1$ no resistor $R_0$.

\begin{equation*}
    E_{th} = V_{R_0} = \dfrac{R_0}{R_0+R_1}e_1 = \dfrac{e_1}{2}
\end{equation*}
...
\end{minipage} \hfill
\begin{minipage}[c][3cm][c]{\dimexpr0.4\textwidth-0.5\Colsep\relax}
    Para encontrar o resistor equivalente de Thévenin em um circuito em notação potencial, pode-se utilizar o método previamente mencionado na Seção \ref{subsubsec:thfontecontrolada}, que consiste em aplicar uma fonte de corrente de $1A$ em que a tensão que cai sobre ela é de valor igual a $R_{th}$. O análogo para notação potencial é mostrado no circuito. Vale salientar que, dado que a fonte $e_1$ foi morta para o cálculo do resistor equivalente, os resistores $R_0$ e $R_1$ agora estão em paralelo.
\end{minipage}
\end{minipage}
...
\begin{equation*}
    R_{th} = 1A\cdot[(R_0//R_1)+R_2] = \frac{3}{2}R
\end{equation*}
...
\begin{minipage}[c][3cm][c]{\dimexpr0.6\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}\draw
            (0,0) node[label={left:$\dfrac{e_1}{2}$}]{} to[R,l=$\frac{3}{2}R$,*-] (2,0)
            (2,0) to[R,l=$R_3$,*-] (4,0)
            (4,0) to[short,f=$\dfrac{\dfrac{e_1}{2}+15}{25k}$,-*] (5,0) node[label={right:$-15V$}]{}
            ;
        \end{tikzpicture}
    \end{center}
\end{minipage} \hfill
...
\begin{equation*}
    -\dfrac{e_0}{10k} = i_{D_1} = \dfrac{1}{10k}[15-\dfrac{20k}{25k}(\dfrac{e_1}{2}+15)]>0\\
\end{equation*}
\begin{equation*}
    -\dfrac{2e_1}{5}+3>0
\end{equation*}
...
\begin{equation*}
    V_{D_2} = -\dfrac{10k}{25k}(\dfrac{e_1}{2}+15)<0\\
\end{equation*}
\begin{equation*}
    -\dfrac{e_1}{2}-15<0
\end{equation*}
...
\begin{equation}
\label{eq:h2}
    e_0 = \dfrac{2e_1}{5}-3, \text{ se }  -30<e_1<7.5
\end{equation}
...
\begin{minipage}[c][3cm][c]{\dimexpr0.6\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}\draw
            ...
            (2,0) to[short,f=$\dfrac{e_1+7.5}{25k}$,-*] (3,0)
...
\begin{equation*}
    \dfrac{e_0}{10k} = i_{D_2} = \dfrac{1}{10k}[e_1-\dfrac{20k}{25k}(\dfrac{e_1}{2}+7.5)]>0
\end{equation*}
\begin{equation*}
    \dfrac{e_1}{5}-6>0
\end{equation*}
...
\begin{equation*}
    V_{D_1} = -\dfrac{10k}{25k}(\dfrac{e_1}{2}+7.5)<0
\end{equation*}
\begin{equation*}
    -e_1-7.5<0
\end{equation*}
...
\begin{equation}
\label{eq:h3}
    e_0 = \dfrac{e_1}{5}-6, \text{ se }  e_1>30
\end{equation}
...
\begin{equation*}
    i_{D_2} = -\dfrac{1}{15k}(\dfrac{e_1}{2}+15)>0
\end{equation*}
...
\begin{equation*}
    e_0 = 10k(i_{D_2}-i_{D_1}) = \dfrac{2}{3}[e_1+7.5-\dfrac{e_1}{2}-15]
\end{equation*}
...
\begin{equation}
\label{eq:h4}
    e_0 = \dfrac{e_1}{3}-5, \text{ se }  e_1<-30
\end{equation}
...
\noindent Por fim, a resposta final para a saída $e_0$ em função de $e_1$ é obtida a partir das expressões \eqref{eq:h1}, \eqref{eq:h2}, \eqref{eq:h3} e \eqref{eq:h4} e é mostrada a seguir.
    ...
    \begin{center}
        ...
        \begin{cases}
            ...
            \frac{1}{3}e_1-5,& \text{ se }  e_1<-30, \text{ onde D1 e D2 \textbf{ON}}\\

            \frac{2}{5}e_1-3,& \text{ se }  -30<e_1<7.5, \text{ onde D1 \textbf{ON} e D2 \textbf{OF}}\\

            0,& \text{ se }  7.5<e_1<30, \text{ onde D1 e D2 \textbf{OFF}}\\

            \frac{1}{5}e_1-6,& \text{ se }  e_1>30, \text{ onde D1 \textbf{OFF} e D2 \textbf{ON}}
...
Existe também um circuito equivalente ao qual procede diretamente de um quadripolo ao qual os parâmetros são dados, porém também é possível fazer o processo inverso, ao encontrar um circuito com esta mesma topologia, e descobrir os parâmetros diretamente dos valores dos resistores. Neste caso, o circuito equivalente fica como segue:
...
\begin{center}
    \begin{tikzpicture}
        \draw
        (0,0) to[short,f=$I_1$,*-] (1,0)
        (1,0) to[R,l_=$\dfrac{1}{y_{11}}$] (1,-2)
        (1,0) -- (3,0)
        (3,-2) to[I,l=$y_{12}E_2$] (3,0)
        (0,0) to[open,v=$V_1$] (0,-2)

        (8,0) to[short,f_=$I_2$,*-] (7,0)
        (7,0) to[R,l=$\dfrac{1}{y_{22}}$] (7,-2)
        (7,0) -- (5,0)
