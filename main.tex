...
\begin{equation}
    ...
    \hspace{0.6cm}
    P_{R} = V_{R}i_{R} \hspace{0.2cm} [W]\footnote{A diferença entre P e Q e suas unidades de medida são devidamente esclarecidas na Seção \ref{subsubsec:unidades}.}
    \hspace{0.6cm}
...
\begin{equation}
    ...
    \hspace{0.6cm}
    Q_{C} = V_{C}i_{C} \hspace{0.2cm} [VAr]
    \hspace{0.6cm}
...
\begin{equation}
    ...
    \hspace{0.6cm}
    Q_{L} = V_{L}i_{L} \hspace{0.2cm} [VAr]
    \hspace{0.6cm}
...
\begin{minipage}[c][5cm]{\dimexpr0.48\textwidth-0.5\Colsep\relax}
...
Elementos ideais cuja função é medir algum valor em algum outro elemento do circuito.\footnote{Conceitos de Impedância, Paralelo e Série são devidamente definidos na Seção \ref{subsec:def}} \\
...
\begin{minipage}[c][7cm][c]{\dimexpr0.55\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}[scale=0.9,transform shape]\draw
    ...
    \end{center}
\end{minipage} \hfill
\begin{minipage}[c][7cm][c]{\dimexpr0.36\textwidth-0.5\Colsep\relax}
...
\noindent\begin{minipage}{0.95\textwidth}
\begin{minipage}[c][8cm][c]{\dimexpr0.55\textwidth-0.5\Colsep\relax}
    \begin{center}
        \begin{tikzpicture}[scale=0.9,transform shape]\draw
    ...
    \end{center}
\end{minipage} \hfill
\begin{minipage}[c][8cm][c]{\dimexpr0.36\textwidth-0.5\Colsep\relax}
...
\vspace{5mm}
\noindent Alternativamente, $i_x$ pode ser reescrito em função de $I$ como a equação de uma reta, isto é, $i_x = a_1I + a_0$, sendo $a_1$ e $a_0$ definidos pela equação
...
Note que ao passar o circuito para notação potencial, ambas as fontes de tensão foram trocadas por potenciais de mesmas respectivas intensidades, visto que seus valores eram dados em relação à referência nula. Da mesma forma, o terminal do resistor na vertical $R$ que o conectava a $e_1$ e $e_2$ foi substituído pelo próprio terra.
...
\begin{center}
    \begin{tikzpicture}\draw
        (0,0) node[op amp] (opamp) {}
        (opamp.+) node[ground] {}
        ($(opamp.+)+(0.3,0)$) to[open,v^=$ $, l=0, voltage shift=1] ($(opamp.-)+(0.3,0)$)
        (opamp.-) to[short,f_<=$0$,current arrow scale=24] (-1.5,0.49) to [R,l=$R_i$,-*,v<=$V_i$,f<=$i$] (-3.5,0.49) node[left] {$V_i$}
        (opamp.out) -- (1.2,1.5) to [R,l_=$R_f$,f_<=$i$] (-1.5,1.5) -- (-1.5,0.49)
        (opamp.out) to [short,-*] (1.8,0) node[right] {$V_o$}
        ;
    \end{tikzpicture}
\end{center}
...
\begin{equation}
    \begin{cases}
        \dfrac{ d^2e_{o}(t) }{ dt }+ 2.5 \frac{ de_{o}(t) }{ dt } + e_{o}(t) = 10\\ \\
...
\end{equation}
\paragraph{}O polinômio acima representa a \textbf{equação característica}, elemento importante no momento de definir conceitos como a resposta natural e a estabilidade de um sistema. A equação característica terá sua ordem definida pela equação diferencial homogênea. No caso em questão, uma EDOL de ordem dois possui duas raízes em sua equação característica.
...
\end{itemize}
%
\large\textbf{Imagem ckt tempo -> frequência}
...
%
\large\textbf{Diagrama da questão da lista}
...
%%
\large\textbf{Imagem de um diagrama de estados "cortado" nas variáveis de estado e renomeado}
